...
namespace TowerCrafter.Grid.Utlis
{
    ...
    {
        public static void MarkSlots(this CustomGrid grid, ItemBase itemData, DragDropUI UI, Vector2 gridPos)
        {
        ...
        public static bool InBounds(this CustomGrid grid, ItemBase item, Vector2Int gridPosition)
        {
        ...
        public static Vector2Int GridPositionFromAnchorPosition(this CustomGrid grid, RectTransform rect)
        {
        ...
        public static Vector2 AnchorPositionFromItem(this CustomGrid grid, ItemBase item)
        {
            return new Vector2(
        ...
        public static Vector2 AnchorPositionFromGridPosition(this CustomGrid grid, Vector2 gridPosition)
        {
            return new Vector2(
                Mathf.FloorToInt(gridPosition.x * grid.Width),
                Mathf.FloorToInt(-gridPosition.y * grid.Height)
            );
        }
        public static void ReturnToOrginalPosition(this CustomGrid grid, DragDropUI dragDrop)
        {
            dragDrop.RectTransform.anchoredPosition = AnchorPositionFromItem(grid, dragDrop.ItemData);
        }
        public static void Place(this CustomGrid grid, DragDropUI dragged, Vector2Int gridPos, int cate = -1, int stash = -1)
        {
            if (InBounds(grid, dragged.ItemData, gridPos))
            {
                ...
                dragged.ItemData.CategoryIndex = cate;
                dragged.ItemData.StashIndex = stash;
                return;
            }
            Debug.LogWarning("Trying to place: " + dragged.name + "at Cord:" + gridPos + " Is out of bounds");
        }
        public static HashSet<DragDropUI> CountOverlaps(this CustomGrid grid, DragDropUI currentlyDragging)
        {
            HashSet<DragDropUI> overlappingItems = new();
        ...
        public static bool CanSwap(this CustomGrid grid, DragDropUI dragged, DragDropUI overlapped)
        {
        ...
        public static bool TryFindFirstAvailablePosition(this CustomGrid grid, ItemBase item, out Vector2Int gridPosition)
        {
